...
function VideoPage(
  ...
  var i = 0;
  const keys = Object.keys(this.pageConfig.stimuli)
  shuffle(keys);
...
VideoPage.prototype.getName = function () {
  ...
  if(this.pageConfig.type == "video_believable"){
    var details = "Crédibilité (Sans audio)"
  }else if(this.pageConfig.type == "video_synchronized"){
    var details = "Coordination (Avec audio)"
  }else{
    return "Page de test"
  }
  return "Page " + this.pageNumber + " sur " + this.pageLenght + " - " + details
};
...
VideoPage.prototype.render = function (_parent) {
  ...
  var p = $("<p><b><i>" + this.pageConfig.question + "<i></b><br>" + this.pageConfig.add + "</p>");
  div.append(p);
  ...
  var tdConditionPlayScale = $("<td>"+this.pageConfig.ref+"</td>");
  ...
  for (i = 0; i < conditions.length; ++i) {
    td = $(
      "<td class='spaceForSlider'> \
        <span class='slider-color-" +conditions[i].color +"'> \
          <input type='range' name='" +conditions[i].getId() +"' class='scales' value='100' min='0' max='100' data-vertical='true' data-highlight='true' style='display : inline-block; float : none;'/>\
          </span> \
      </td>"
    );
    td.style = conditions[i].color
  ...
  this.videoVisualizer = new VideoVisualizer(tableUp, this.conditions, this.audio);
